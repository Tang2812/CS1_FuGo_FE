...
const HomeSuggest = () => {
    ...
    const handleBeforeUnload = async (event) => {
        ...
        try {
            ...
            const mostClickedJob = Object.entries(parsedData).reduce((max, entry) => {
                const [jobId, clickCount] = entry;
                return clickCount > max.clickCount ? {jobId, clickCount} : max;
            }, {jobId: null, clickCount: 0});

            const jobToRecommend = jobs.find(job => job._id === mostClickedJob.jobId);
            if (jobToRecommend) {
                data.selected_jobs.push(jobToRecommend);
            }
            console.log("data: ", data);
            // Gửi dữ liệu
            await axios.post("http://localhost:3000/api/v1/jobsRecommend/insert", data);
